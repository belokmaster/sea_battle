...
void Bombard::apply(BattleField& field, int x, int y, ShipManager& manager) {
    srand(time(NULL));
    ...
    if (manager.getShipsCount() > 0) {
        ...
        std::cout << "Bombard dealt damage to ship at number " << random_index + 1 << " and segment " << segment_index + 1 << std::endl;
    } 
}